...
class PostMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            val publishers = response.message.posts.map {
                it.publisher
            }
            database.withTransaction {
                ...
                postDao.insertAll(response.message.posts)
                publisherDao.insertAll(publishers as List<PublisherModel>)
                response.message.posts.forEach { post ->
                    post.photos?.forEach {
                        photoDao.insert(PhotoModel(photo = it, post_id = post.id))
                    }
                }
                remoteKeyDao.insert(RemoteKey("post", response.message.next))
            }
            return MediatorResult.Success(
                endOfPaginationReached = response.message.next == null,
            )
